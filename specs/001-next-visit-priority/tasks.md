...
- [X] T026 [US1] Run `pnpm test:e2e -- story1-queue` against dev server and iterate until all assertions in `tests/e2e/story1-queue.spec.ts` pass; add a Playwright timing assertion that queue visibility occurs within 5 000 ms using `performance.now()` or Playwright `clock` (automated SC-001 check)
